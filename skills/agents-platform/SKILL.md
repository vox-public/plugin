...
description: "연결된 vox.ai Agents MCP에서 조직·도구를 확인하고 에이전트·Manual·통화·번호·캠페인 업무 도구를 사용할 때 적용한다. 향후 desk/operator MCP의 실시간 상담 업무에는 적용하지 않는다."
metadata:
  product: vox.ai
  layer: mcp
...
## 제품 모듈 선택
이 원고는 Agents 제품의 연결과 도구 의미를 소유한다. 사용자의 요청이 음성 agent 구축·시험·통신 운영이면 이 모듈을 선택한다. desk의 상담원 수신·이어받기·상담원 발신 요청을 비슷한 이름의 Agents 도구에 넘기지 않는다. desk 모듈은 아직 배포되지 않았으므로 현재 연결에서 지원 여부를 확인하고 미지원 범위를 알려준다.

## 연결과 발견
외부 대화형 연결은 원격 OAuth, 내장 코파일럿은 같은 원격 MCP에 사용자 위임, 서비스 자동화는 조직 API 키라는 설계를 따른다. 키나 토큰을 프롬프트/파일에 요구하지 않는다. 연결 조직은 고정이며 모델 인자로 전환하지 않는다. 조직 변경은 호스트의 연결 UX로 처리한다.
