...
Manual 본문 수정이면 연결된 Manual의 정확한 ID로 `get_manual`, 도구 오류면 `get_tool`, 과거 콜 분석이면 `get_call`과 당시 버전의 조회 지원 범위를 확인한다. independent 조회만 병렬화하고 이전 응답 ID가 필요한 조회는 기다린다.

## 결과
agent/대상 버전/관련 참조/현재값/불명 정보를 정리한다. 목록 응답에 본문이 없거나 반환이 잘렸다면 빈 설정으로 해석하지 않는다. 전체 교체에 필요한 완전한 원문이 없으면 해당 저장을 진행하지 않는다.
