...
## 변경
`list_models`와 실제 schema로 지원 조합을 확인한다. 코파일럿을 실행하는 모델과 고객과 대화하는 음성 agent의 모델을 혼동하지 않는다. 가설에 필요한 설정만 `save_agent`로 바꾸고 같은 묶음의 나머지 값을 보존한다. 지원되지 않는 설정명이나 기본값을 추정하지 않는다.
